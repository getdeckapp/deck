<x-deck::poll-container :enabled="$shouldPoll" :seconds="$pollSeconds">
    ...
    <dl class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <x-deck::stat-card label="Avg duration ({{ $rollupHours }}h)" :value="\Deck\Deck\Presentation\FormatDuration::format($runtimeRollup->avgMs)" />
        <x-deck::stat-card label="p50 ({{ $rollupHours }}h)" :value="\Deck\Deck\Presentation\FormatDuration::format($runtimeRollup->p50Ms)" />
        <x-deck::stat-card label="p95 ({{ $rollupHours }}h)" :value="\Deck\Deck\Presentation\FormatDuration::format($runtimeRollup->p95Ms)" />
