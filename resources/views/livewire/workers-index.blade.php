<div @if($shouldPoll) wire:poll.10s @endif class="space-y-8">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-white">Workers & queues</h1>
    ...
    @if ($horizonAvailable && count($horizonSupervisors) > 0)
        <section class="overflow-hidden rounded-2xl border border-zinc-200/60 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.06)] dark:border-zinc-800 dark:bg-zinc-900">
            <div class="border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">Supervisors</h2>
                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Each supervisor manages worker processes for one or more queues.</p>
            </div>
                ...
                <table class="min-w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        @foreach ($horizonSupervisors as $supervisor)
                            <tr>
                                <td class="py-3 font-medium text-zinc-900 dark:text-white">{{ $supervisor['name'] }}</td>
                                <td class="py-3 text-zinc-600 dark:text-zinc-400">{{ $supervisor['master'] }}</td>
                                <td class="py-3 capitalize text-zinc-600 dark:text-zinc-400">{{ $supervisor['status'] }}</td>
                                <td class="py-3 tabular-nums text-zinc-600 dark:text-zinc-400">{{ $supervisor['processes'] }}</td>
                                <td class="py-3 text-zinc-600 dark:text-zinc-400">
                                    @foreach ($supervisor['queues'] as $queue)
                                        <span class="mr-2 inline-flex rounded-md bg-zinc-100 px-2 py-0.5 text-xs dark:bg-zinc-800">
