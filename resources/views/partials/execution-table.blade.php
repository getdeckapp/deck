...
@endphp
<div
    class="overflow-hidden rounded-2xl border border-zinc-200/60 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.06)]">
    <table class="min-w-full divide-y divide-zinc-200">
        ...
        <tr>
            ...
            @endif
            <th scope="col" @class(['py-3.5 pr-3 text-left text-sm font-semibold text-zinc-900', 'pl-4 sm:pl-6' => ! $showJobClass, 'px-3' => $showJobClass])>
                Status
            </th>
            ...
            <th scope="col"
                class="relative py-3.5 pr-4 pl-3 text-right text-sm font-semibold text-zinc-900 sm:pr-6"></th>
        </tr>
        </thead>
            ...
            >
                ...
                <td @class(['py-4 pr-3 text-sm', 'pl-4 sm:pl-6' => ! $showJobClass, 'px-3' => $showJobClass])>
                    <div class="flex flex-wrap items-center gap-2">
                        <x-deck::badge
                            :status="$execution->status->value">{{ $execution->status->value }}</x-deck::badge>
                        @if ($execution->isCancellationPending())
                            @include('deck::partials.cancellation-pending-badge')
                        @endif
                        @if ($execution->isLongRunning())
                            <span
                                class="rounded-md bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-600/20">Long running</span>
                        @endif
                        @if ($execution->attempt > 1)
                            <span
                                class="rounded-md bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-700 ring-1 ring-inset ring-zinc-200">Attempt {{ $execution->attempt }}</span>
                            <span
                                class="rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-600/20">Retry #{{ $execution->attempt - 1 }}</span>
                        @endif
                    </div>
                    @if (! empty($execution->tags))
                        <div class="mt-2 flex flex-wrap gap-1">
                            @foreach ($execution->tags as $tag)
                                <span
                                    class="rounded bg-zinc-100 px-1.5 py-0.5 font-mono text-xs text-zinc-600">{{ $tag }}</span>
                            @endforeach
                        </div>
                    @endif
                ...
                <td class="px-3 py-4 text-sm whitespace-nowrap text-zinc-500">
                    <time
                        datetime="{{ $execution->started_at->toIso8601String() }}">{{ $execution->started_at->format('M j, H:i:s') }}</time>
        ...
        @empty
            <tr>
                <td colspan="{{ $showJobClass ? 6 : 5 }}"
                    class="py-12 text-center text-sm text-zinc-500">{{ $emptyMessage }}</td>
            </tr>
        @endforelse
        </tbody>
    </table>
