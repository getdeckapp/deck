@if ($pendingConfirmation)
    @php
        ...
        $confirmButton = match ($tone) {
            ...
            'warning' => 'bg-amber-600 text-white hover:bg-amber-500 focus-visible:outline-amber-600 dark:bg-amber-500 dark:hover:bg-amber-400',
            default => 'bg-indigo-600 text-white hover:bg-indigo-500 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400',
        };
    @endphp
    ...
    <div
        ...
        wire:key="deck-confirm-{{ $pendingConfirmation['method'] }}-{{ md5(json_encode($pendingConfirmation['arguments'])) }}"
    >
        <div
            class="fixed inset-0 bg-zinc-900/50 backdrop-blur-sm transition-opacity dark:bg-black/60"
        ...
        <div class="relative w-full max-w-md overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-2xl ring-1 ring-black/5 dark:border-zinc-700 dark:bg-zinc-900 dark:shadow-black/50 dark:ring-white/10">
            <div class="p-6">
                <div class="flex gap-4">
                    ...
                    <div class="min-w-0 flex-1 pt-0.5">
                        <h3 id="deck-confirm-title" class="text-base font-semibold text-zinc-900 dark:text-white">
                            {{ $pendingConfirmation['title'] }}
                        </h3>
                        <p id="deck-confirm-message" class="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
                ...
                <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                    <button
                        type="button"
                        class="inline-flex justify-center rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-zinc-900 shadow-xs ring-1 ring-inset ring-zinc-300 hover:bg-zinc-50 disabled:opacity-50 dark:bg-zinc-800 dark:text-white dark:ring-zinc-600 dark:hover:bg-zinc-700"
                        wire:click="cancelConfirmation"
