...
<div class="overflow-hidden rounded-2xl border border-zinc-200/60 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.06)] dark:border-zinc-800 dark:bg-zinc-900">
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
        <div>
        ...
        @if ($horizonAvailable && $horizonSummary)
            <div class="flex flex-wrap items-center gap-2">
                <span @class([
                    ...
                    'bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-500/10 dark:text-emerald-300' => $horizonSummary['status'] === 'running',
                    'bg-amber-50 text-amber-700 ring-amber-600/20 dark:bg-amber-500/10 dark:text-amber-300' => $horizonSummary['status'] === 'paused',
                    'bg-zinc-100 text-zinc-600 ring-zinc-500/20 dark:bg-zinc-800 dark:text-zinc-400' => $horizonSummary['status'] === 'inactive',
                ])>
                    <span class="size-1.5 rounded-full bg-current"></span>
                    {{ ucfirst($horizonSummary['status']) }}
                </span>
                <span class="rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600 ring-1 ring-zinc-200/80 dark:bg-zinc-800 dark:text-zinc-300 dark:ring-zinc-700">
    ...
    @if ($horizonAvailable)
        <div class="grid gap-6 p-5 lg:grid-cols-2">
            ...
            <div>
                ...
                @else
                    <ul class="divide-y divide-zinc-100 rounded-xl border border-zinc-200/60 dark:divide-zinc-800 dark:border-zinc-800">
                        @foreach ($horizonMasters as $master)
                            <li class="flex items-center justify-between px-4 py-3 text-sm">
                                <div>
                                    <p class="font-medium text-zinc-900 dark:text-white">{{ $master['name'] }}</p>
                                    <p class="text-xs text-zinc-500">{{ $master['supervisors'] }} supervisors</p>
                                </div>
                                <div class="text-right">
                                    <p class="font-medium tabular-nums text-zinc-700 dark:text-zinc-300">{{ $master['processes'] }} processes</p>
                ...
                <table class="min-w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        @foreach ($queueInsights as $queue)
                            <tr>
                                <td class="py-3 font-medium text-zinc-900 dark:text-white">{{ $queue['queue'] }}</td>
                                <td class="py-3 tabular-nums text-zinc-600 dark:text-zinc-400">{{ $queue['running'] }}</td>
                                <td class="py-3 tabular-nums text-zinc-600 dark:text-zinc-400">{{ $queue['completed_last_hour'] }}</td>
                                <td class="py-3 text-zinc-600 dark:text-zinc-400">{{ $queue['avg_duration_ms'] !== null ? \TorMorten\Deck\Support\FormatDuration::format($queue['avg_duration_ms']) : '—' }}</td>
                                <td class="py-3">
